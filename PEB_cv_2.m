...
% From this will select only 16 participants  (Patients in study 1 only) to
% find difference between placebo responders and non responders 

parts_choosen = find(data_sheet(:,1) == 1); 

data_sheet = data_sheet(parts_choosen,:);
...
for ii = 1:length(covariate)
    ...
    % second column for responders (1) vs no responders (-1) 
    
    if data_sheet(ii,5) == 0
        covariate(ii,2) = -1
    elseif  data_sheet(ii,5) == 1
        covariate(ii,2) = 1
    end 
...
GCM_R_three =   GCM_R_three(parts_choosen)
...
[qE,qC,Q]=spm_dcm_loo(GCM_R_three,M,{'A(2,2)'});
...
GCM_L_three = GCM_L_three(parts_choosen)
...
[qE,qC,Q]=spm_dcm_loo(GCM_L_three,M,{'A(2,2)'});
